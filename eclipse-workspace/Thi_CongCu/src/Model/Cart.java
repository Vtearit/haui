...
public class Cart {
	...
	public int totalCart() {
		int count = 0;
		for (Entry<String, Item> listMap : cartItems.entrySet()) {
			if (listMap.getValue().getProduct().getPercent_promotion() > 0) {
				int price_promotion = listMap.getValue().getProduct().getPrice()
						- listMap.getValue().getProduct().getPrice()
								* listMap.getValue().getProduct().getPercent_promotion() / 100;
