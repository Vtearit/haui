		...
		try {
			...
			case "plus":
				if (cart.getCartItems().containsKey(productID)) {
					cart.insertToCart(productID,
							new Item(product, cart.getCartItems().get(productID).getQuantity() + quantity - 1));
				} else {
			...
			case "add":
				cart.insertToCart(productID, new Item(product, cart.getCartItems().get(productID).getQuantity()));
				resp.sendRedirect("Views/Cart.jsp");
				break;
