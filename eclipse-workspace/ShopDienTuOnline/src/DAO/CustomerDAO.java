...
public class CustomerDAO {
	public static Customer customerByID(int ID) throws SQLException {
		...
		while (resultSet.next()) {
			customer = new Customer(resultSet.getInt("id"), resultSet.getString("name"), resultSet.getString("phone"),
	...
	public boolean UpdateProfile(String name, String phone, String address, int id) throws SQLException {
		DBConnect dbConnect = new DBConnect();
		Statement statement = dbConnect.getConnection().createStatement();
		String sql = String.format("Update dbo.Customer set name= N'" + name + "', address=N'" + address + "', phone='"
				+ phone + "' where id= " + id);
